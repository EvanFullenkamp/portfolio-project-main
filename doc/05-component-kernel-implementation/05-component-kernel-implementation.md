...
For the CardDeck implementation I want to have a sequence of cards, Sequence<Card>.
Card represents a single card with a suit and rank enum. I chose sequence because allows
me to easily add and remove any of the cards in the deck, most importantly the top and
bottom cards.
Convention: deck is not null and contains no null cards
Correspondence: The abstract deck is represented by the order of items in deck: the i-th
card of the mathematical model is stored in deck.entry(i).
Thus, the sequence <deck.entry(0), …, deck.entry(n−1)> corresponds directly to the mathematical deck.
